...
def _distill_cloud(heading: str, text: str) -> list[str]:
    ...
    except Exception as e:  # noqa: BLE001 — surface and skip the section
        print(f"  ! cloud extract failed on {heading!r}: {e!r}")
        return []
...
def distill(src: Path, out: Path, backend: str = "heuristic") -> dict[str, Any]:
    ...
    return {"backend": backend, "sections_used": n_sections, "units": n_units, "out": str(out)}


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("src", help="source .md to distill")
